...
for ( species in all_stock) {
  ...
  for (parent in parents) {
    ...
    for (i in 1:nrow(data_matrix)) {
      ...
      if(any(data_matrix[i, ] > 60, na.rm = TRUE)) {
    ...
    data_grouped <- data_adjusted %>%
      ...
      summarise(across(everything(), list(mean = ~mean(.x, na.rm = TRUE), sd = ~sd(.x, na.rm = TRUE))), .groups = 'drop') %>%
      ungroup()
    data_grouped <- data_adjusted %>%
      group_by(!!sym(birth_month_col)) %>%
      summarise(
    ...
    # Graphs with SD as error bar
    for (i in 1:12) {
      # Get the full month name
      full_month_name <- month.name[i]
      
      # Dynamically generate column names for mean and standard deviation
      column_name_mean <- paste0("X", i, "_mean")
      column_name_sd <- paste0("X", i,"_sd")
      
      # Create the plot with dynamic y values based on the month
      p <- ggplot(data_grouped, aes(x = !!sym(birth_month_col), y = !!sym(column_name_mean))) + 
        geom_point(color = "blue", size = 4) +
        geom_errorbar(color = "blue",
          aes(
            ymin = !!sym(column_name_mean) - !!sym(column_name_sd), 
            ymax = !!sym(column_name_mean) + !!sym(column_name_sd)
          ),
          width = 0.4
        ) +
        theme_minimal() +
      ...
      ggsave(
        paste0('./graph/SD/(SD) ',species, ' - ', parent, ' born in ', i ,' (', full_month_name, ").png"), 
        plot = p, 
        width = 8, 
        height = 6
      )
    ...
    for (i in 1:12) {
      ...
      # Dynamically generate column names for mean and standard deviation
      column_name_mean <- paste0("X", i, "_mean")
      column_name_sd <- paste0("X", i,"_sem")
      # Create the plot with dynamic y values based on the month
      p <- ggplot(data_grouped, aes(x = !!sym(birth_month_col), y = !!sym(column_name_mean))) + 
        geom_point(color = "blue", size = 4) +
        geom_errorbar(color = "blue",
