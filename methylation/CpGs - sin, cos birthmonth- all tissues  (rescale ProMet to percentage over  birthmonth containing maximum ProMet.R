...
myscan = nbs %>%
  ...
  mutate(logp = map(data, ~ -log(anova(
    lm(ProMet ~ Age + Sex + Tissue + SpeciesAbbreviation, data = .),
    lm(ProMet ~ Month_sin + Month_cos + Age + Sex + Tissue + SpeciesAbbreviation, data = .)
  )[[6]][2])/log(10))) %>%
  unnest(logp)
...
write.xlsx(BW_scan, './data/BWanno - CpGs - sin, cos - circannual birthmonth - all tissues.xlsx')
...
ggsave("./data/methylation level across birthmonth/all tissues - all.png", plot = plot_all, width = 10, height = 8, dpi = 300)
...
# Save the plot with a full path specification
ggsave("./data/methylation level across birthmonth/all tissues - high in Sep.png", plot = plot_high, width = 10, height = 8, dpi = 300)
...
# Save the plot with a full path specification
ggsave("./data/methylation level across birthmonth/all tissues - low in Sep.png", plot = plot_low, width = 10, height = 8, dpi = 300)
...
# Save the plot with a full path specification
ggsave("./data/methylation level across birthmonth/all tissues - middle in Sep.png", plot = plot_middle, width = 10, height = 8, dpi = 300)
...
write.xlsx(summary_table, "./data/methylation level across birthmonth/summary_table - all tissues.xlsx")
