...
SS=read_xlsx("all_DNAm_mice_all_tissues.xlsx") %>% distinct() %>%
  mutate(SpeciesAbbreviation = if_else(is.na(SpeciesAbbreviation),
                                       "Hybrid", SpeciesAbbreviation)) %>%
...
write.xlsx(BW_scan, './data/BWanno - CpGs - sin, cos - circannual birthmonth - male tails.xlsx')
########## select top CpGs with p-value and FDR < 0.05


top_cpg = myscan %>% filter(p < 0.05) 
nrow(top_cpg)
...
ggsave("./data/methylation level across birthmonth/male tails - all.png", plot = plot_all, width = 10, height = 8, dpi = 300)
...
# Save the plot with a full path specification
ggsave("./data/methylation level across birthmonth/male tails - high in Sep.png", plot = plot_high, width = 10, height = 8, dpi = 300)
...
# Save the plot with a full path specification
ggsave("./data/methylation level across birthmonth/male tails - low in Sep.png", plot = plot_low, width = 10, height = 8, dpi = 300)
...
# Save the plot with a full path specification
ggsave("./data/methylation level across birthmonth/male tails - middle in Sep.png", plot = plot_middle, width = 10, height = 8, dpi = 300)
...
write.xlsx(summary_table, "./data/methylation level across birthmonth/summary_table - male tails.xlsx")
