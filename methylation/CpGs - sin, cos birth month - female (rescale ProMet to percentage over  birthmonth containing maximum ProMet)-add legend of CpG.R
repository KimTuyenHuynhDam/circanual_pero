...
#on Window
#on Mac
setwd("~/Library/CloudStorage/OneDrive-UniversityofSouthCarolina/Kiaris lab/peromyscus/data for MS seasonality 2024/circanual/methylation")
...
myscan = nbs %>%
  ...
  mutate(logp = map(data, ~ -log(anova(
    lm(ProMet ~ Age  + SpeciesAbbreviation, data = .),
    lm(ProMet ~ Month_sin + Month_cos + Age  + SpeciesAbbreviation, data = .)
  )[[6]][2])/log(10))) %>%
  unnest(logp)
...
#write.xlsx(BW_scan, './data/BWanno - CpGs - sin, cos - circannual birthmonth - female tails.xlsx')

gene_cg = anno_BW %>% select (CGid, SYMBOL) %>%
  rename(cg = CGid)

################
graph <- function(top_cpg) {
...
middle_september_met = group_by_birthmonth_test[1]
 ...
 # write.xlsx(summary_table, "./data/methylation level across birthmonth/summary_table - female tails.xlsx")
  ...
  # Create a new column combining 'cg' and 'SYMBOL'
  data_long$`cg(gene)` <- paste(data_long$cg, " (", data_long$SYMBOL, ")", sep = "")
  
  
  
  plot_all <- ggplot(data_long, aes(BirthMonth,value)) +  theme_minimal() +
   # geom_line(aes(linetype = `cg(gene)`)) +  # Adding linetype aesthetic based on 'cg' and 'SYMBOL'
    geom_line(aes(colour = `cg(gene)`), linewidth =1.5) +  # Adding color aesthetic based on 'cg' and 'SYMBOL'
    theme(legend.position = "right", # Adjust legend position as needed,
          legend.text = element_text(size = 24),
          legend.title = element_blank(),  # Remove legend title
          axis.text.x = element_text(size = 24, hjust = 1),   # Custom font size for x-axis ticks
          axis.text.y = element_text(size = 24),                         # Custom font size for y-axis ticks
          axis.title.x = element_text(size = 24, face = "bold"),# Bold x-axis label with larger font
  ...
  ggsave("./data/methylation level across birthmonth/female tails - all -with legend.png", plot = plot_all, width = 14, height = 8, dpi = 300)
  ...
  # Save the plot with a full path specification
  ggsave("./data/methylation level across birthmonth/female tails - middle in Sep.png", plot = plot_middle, width = 10, height = 8, dpi = 300)
